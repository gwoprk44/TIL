...
- 새로운 엔티티가 아니면 병합한다.
  - 기존 값을 새로운 엔티티로 교체한다.
  - DB에서 select를 무조건 한 번 한다는 단점이 존재한다.
  - 데이터 변경은 병합보다 변경 감지를 활용하도록 하자.
...
```
- 식별자에 값이 존재하므로 병합한다.
- DB에 값이 있다는 걸 가정하므로 일단 존재하는지 쿼리를 날린다.
...
```
- 어떤 이슈로 임의의 ID를 직접 생성해야 한다면 Persistable를 implement한다.
```java
@Entity
@EntityListeners(AuditingEntityListener.class)
@NoArgsConstructor(access = AccessLevel.PROTECTED)
